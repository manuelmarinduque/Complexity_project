...
class LinearProgramming():
    ...
    def __construirTabla(self):
        ...
        df = pd.DataFrame(info, columns=('Población', 'Estaciones', 'Personal', 'Costos', 'Muertes', 'Cualificación'))
        return df
    ...
    def __generarTablaModelo(self, tabla):
        ...
        cualificacion = tabla['Cualificación'].values
    ...
    def modelo(self, tabla, total_ventiladores=1000, total_personal=2000, total_costo=3500000000, total_cualificacion=1000):
        ...
        model.solve()
        return value(regiones[0]), value(regiones[1]), value(regiones[2]), value(regiones[3]), value(regiones[4]), value(regiones[5]), value(regiones[6]), value(model.objective)

objeto = LinearProgramming(num_regiones=7)
tabla = objeto.generarTabla()
resultado = objeto.modelo(tabla)
print(resultado)
